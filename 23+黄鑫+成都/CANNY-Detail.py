...
if __name__ == '__main__':  # 当做脚本时才能执行，import不能执行
    img_path = "C:\\Users\\LENOVO\\Desktop\\lenna.png"
    ...
    # 高斯平滑
    # siqma =1.52 #高斯平滑时的高斯核参数，标准差，可调
    sigma = 0.5  # 高斯平滑时的高斯核参数，标准差，可调
    dim = int(np.round(6 * sigma + 1))  # round是四舍五入函数，根据标准差求高斯核是几乘几，也就是维度
    if dim % 2 == 0:  # 最好是奇数，不是的话加一
        dim += 1
    Gaussian_filter = np.zeros([dim, dim])  # 储存高斯核，这是数组不是列表
    ...
    dx, dy = img.shape
    img_new = np.zeros(img.shape)  # 存储平滑之后的图像,zeros函数得到的是浮点类型
    tmp = dim // 2
    ...
    for i in range(1, dx - 1):
        for j in range(1, dy - 1):
            flag = True  # 判断在8领域内是否要抹去做个标记
            temp = img_tidu[i - 1:i + 2, j - 1:j + 2]  # 梯度幅值8领域矩阵
    ...
    while not len(zhan) == 0:
        ...
        if (a[0, 0] < high_boundary) and (a[0, 0] > lower_boundary):
            img_yizhi[temp_1 - 1, temp_2 - 1] = high_boundary  # 这个像素点标记为边缘
            zhan.append([temp_1 - 1, temp_2 - 1])  # 进栈
        if (a[0, 1] < high_boundary) and (a[0, 1] > lower_boundary):
            img_yizhi[temp_1 - 1, temp_2] = high_boundary
            zhan.append([temp_1 - 1, temp_2])
        if (a[0, 2] < high_boundary) and (a[0, 2] > lower_boundary):
            img_yizhi[temp_1 - 1, temp_2 + 1] = high_boundary
            zhan.append([temp_1 - 1, temp_2 + 1])
        if (a[1, 0] < high_boundary) and (a[1, 0] > lower_boundary):
            img_yizhi[temp_1, temp_2 - 1] = high_boundary
            zhan.append([temp_1, temp_2 - 1])
        if (a[1, 2] < high_boundary) and (a[1, 2] > lower_boundary):
            img_yizhi[temp_1, temp_2] = high_boundary
