...
if __name__ == '__main__':
    ...
    if pic_path[-4:] == '.png':  # .png图片在这里的存储格式是0到1的浮点数，所以要扩展到255再计算
        img = img * 255  # 还是浮点数类型
    img = np.average(img,axis=-1)  # 取均值就是灰度化了

    # Step 2 对图像进行高斯滤波（加权平均）
    # 高斯平滑
    ...
    # Step 3  检测图像中的水平、垂直和对角边缘
    # 求梯度。以下两个是滤波求梯度用的sobel矩阵
    ...
    # Step 5  双阈值检测，连接边缘。遍历所有一定是边的点,查看8邻域是否存在有可能是边的点，进栈
