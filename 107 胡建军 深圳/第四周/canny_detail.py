...
if __name__ == '__main__':
    ...
    img = img.mean(axis=-1) # 按最后一维取均值，就是灰度化了
    # print(img[:, :, 0])
    # plt.imshow(img)
    # plt.show()
    ...
    Gaussian_filter = Gaussian_filter / Gaussian_filter.sum()  #归一化，所有值之和为1
    dx, dy = img.shape
    img_new = np.zeros(img.shape)  # 存储平滑之后的图像，zeros函数得到的是浮点型数据
    tmp = dim // 2
    img_pad = np.pad(img, ((tmp, tmp), (tmp, tmp)), 'constant', constant_values=0)  # 边缘填补
    # print(img_pad)
    for i in range(dx):
        for j in range(dy):
    ...
    for i in range(dx):
        for j in range(dy):
            ...
            img_tidu[i, j] = np.sqrt(img_tidu_x[i, j] ** 2 + img_tidu_y[i, j] ** 2) #总梯度
    img_tidu_x[img_tidu_x == 0] = 0.00000001 #0值处理
    angle = img_tidu_y / img_tidu_x
    plt.figure(2)
    plt.imshow(img_tidu.astype(np.uint8), cmap='gray')
    plt.axis('off')
    plt.title('sobel滤波梯度图')
    # plt.show()
    ...
    for i in range(1, dx - 1):
        for j in range(1, dy - 1):
            flag = True  # 标记，在8邻域内是否要抹去
            temp = img_tidu[i - 1:i + 2, j - 1:j + 2]  # 梯度幅值的8邻域矩阵
            if angle[i, j] <= -1:  # 使用线性插值法判断抑制与否  左上角右下角
                num_1 = (temp[0, 1] - temp[0, 0]) / angle[i, j] + temp[0, 1]
                num_2 = (temp[2, 1] - temp[2, 2]) / angle[i, j] + temp[2, 1]
                if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                    flag = False
            elif angle[i, j] >= 1:  #右上角左下角
                num_1 = (temp[0, 2] - temp[0, 1]) / angle[i, j] + temp[0, 1]
                num_2 = (temp[2, 0] - temp[2, 1]) / angle[i, j] + temp[2, 1]
                if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                    flag = False
            elif angle[i, j] > 0: #（0-1）右上左下
                num_1 = (temp[0, 2] - temp[1, 2]) * angle[i, j] + temp[1, 2]
                num_2 = (temp[2, 0] - temp[1, 0]) * angle[i, j] + temp[1, 0]
                if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                    flag = False
            elif angle[i, j] < 0: #（0：-1）左上右下
    ...
    # 4、双阈值检测，连接边缘。遍历所有一定是边的点,查看8邻++域是否存在有可能是边的点，进栈
