...
plt.figure(1)
plt.imshow(img_gray.astype(np.uint8), cmap='gray')  # 此时的img_new是255的浮点型数据，强制类型转换才可以，gray灰阶
plt.axis('off')
plt.show()
# 对图像进行高斯滤波
...
img_gaussian_filter = np.zeros(img_gray.shape)
dx, dy = img_gray.shape
tmp = dim//2
img_pad = np.pad(img_gray, ((tmp, tmp), (tmp, tmp)), 'constant')  # 边缘填补
for i in range(dx):
    for j in range(dy):
        img_gaussian_filter[i, j] = np.sum(img_pad[i:i+dim, j:j+dim] * Gaussian_filter)
plt.figure(2)
plt.imshow(img_gaussian_filter.astype(np.uint8), cmap='gray')  # 此时的img_new是255的浮点型数据，强制类型转换才可以，gray灰阶
plt.axis('off')
plt.show()
...
img_tidu_x = np.zeros(img_gaussian_filter.shape)  # 存储梯度图像
img_tidu_y = np.zeros(img_gaussian_filter.shape)
img_tidu = np.zeros(img_gaussian_filter.shape)
img_pad = np.pad(img_gaussian_filter, ((1, 1), (1, 1)), 'constant')  # 边缘填补，根据上面矩阵结构所以写1
for i in range(dx):
    for j in range(dy):
...
for i in range(1, dx - 1):
    for j in range(1, dy - 1):
        ...
        if angle[i, j] <= -1:  # 使用线性插值法判断抑制与否  (angle=(1-0)/(1-x))
...
plt.figure(4)
plt.imshow(img_yizhi.astype(np.uint8), cmap='gray')  # 此时的img_new是255的浮点型数据，强制类型转换才可以，gray灰阶
plt.axis('off')
plt.show()

# 双阈值检测
lower_boundary = img_tidu.mean() * 0.5 # 设置高阈值是均值的一半
