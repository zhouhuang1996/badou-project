...
if __name__ == '__main__':
    ...
    if pic_path[-4:] == '.png': # 以文件头的方式快速判断文件格式  # .png图片在这里的存储格式是0到1的浮点数，所以要扩展到255再计算
        img = img * 255  # 还是浮点数类型
    ...
    Gaussian_filter = Gaussian_filter / Gaussian_filter.sum()    # 归一化后的高斯卷积核
    dx, dy = img.shape
    img_new = np.zeros(img.shape)  # 存储平滑之后的图像，zeros函数得到的是浮点型数据
    print("img_new的大小：", img_new.shape)
    tmp = dim//2
    img_pad = np.pad(img, ((tmp, tmp), (tmp, tmp)), 'constant')  # 边缘填补
    for i in range(dx):
        for j in range(dy):
            img_new[i, j] = np.sum(img_pad[i:i+dim, j:j+dim]*Gaussian_filter)  # 计算卷积后的结果
    plt.figure(1)
    ...
    print("img_tidu_x的大小：", img_tidu_x.shape)
    img_tidu_y = np.zeros([dx, dy])   # 存储梯度图像
    print("img_tidu_y的大小：", img_tidu_y.shape)
    ...
    for i in range(dx):
        for j in range(dy):
            ...
            img_tidu[i, j] = np.sqrt(img_tidu_x[i, j]**2 + img_tidu_y[i, j]**2)  # 梯度
    img_tidu_x[img_tidu_x == 0] = 0.00000001   # 如果向梯度为0，则赋值为极小的非零值
    angle = img_tidu_y/img_tidu_x   # 角度
    plt.figure(2)
