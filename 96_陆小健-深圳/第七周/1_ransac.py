...
def ransac(data, model, n, k, t, d, debug = False, return_all = False):
    ...
    while iterations < k:
        ...
        maybe_inliers = data[maybe_idxs, :] #获取size(maybe_idxs)行数据(Xi,Yi),初始为50个数据
        # print('data = ',data)
        ...
        also_idxs = test_idxs[test_err < t]   # 满足test_err < t的test_idxs索引值
        # print ('also_idxs = ', also_idxs)
        also_inliers = data[also_idxs,:]   # also_idxs索引值所对应的data值
        if debug:
...
class LinearLeastSquareModel:
    ...
    def fit(self, data):
        ...
        x, resids, rank, s = sl.lstsq(A, B) #residues:残差和sl.lstsq
        return x #返回最小平方和向量   
...
def test():
    ...
    perfect_fit = 60 * np.random.normal( size = (n_inputs, n_outputs) ) #随机线性度，即随机生成一个0-60的斜率
    ...
    if 1:
        #添加"局外点"
        n_outliers = 100
        all_idxs = np.arange( A_noisy.shape[0] ) #np.arange获取索引0-499的列表
    ...
    #setup model 
    all_data = np.hstack( (A_noisy, B_noisy) ) #np.hstack形式([Xi,Yi]....) shape:(500,2)500行2列
    input_columns = range(n_inputs)  #数组的第一列x:0
    output_columns = [n_inputs + i for i in range(n_outputs)] #数组最后一列y:1
    debug = False
    model = LinearLeastSquareModel(input_columns, output_columns, debug = debug) #类的实例化:用最小二乘生成已知模型
 
    linear_fit,resids,rank,s = sp.linalg.lstsq(all_data[:,input_columns], all_data[:,output_columns])  #residues:残差和sl.lstsq
    
    #run RANSAC 算法
    ransac_fit, ransac_data = ransac(all_data, model, 50, 1000, 7e3, 300, debug = debug, return_all = True)
    print('ransac_fit = ',ransac_fit)
    print('ransac_data = ',ransac_data)
 
    if 1:
        sort_idxs = np.argsort(A_exact[:,0])   # 将A_exact中的元素从小到大排列，提取其对应的index(索引)，然后输出到sort_idxs
        # print('sort_idxs = ', sort_idxs)
        A_col0_sorted = A_exact[sort_idxs] #秩为2的数组
        print('A_col0_sorted = ', A_col0_sorted)
        ...
        pylab.legend()
        pylab.savefig('result.jpg')   # 需要放到show()前面，不然会保存不了目标图像
        pylab.show()
