# canny边缘检测分为4步：1对灰度图作高斯滤波让其各部分平滑
                     # 2求图像的梯度，用sobel矩阵（检测图像中的水平、垂直和对角边缘）
                    #  3求非最大值抑制
                    #  4双阈值检测，可以剔除不需要的边缘点
import numpy as np
...
if __name__ == '__main__':
    ...
    ax1.title.set_text('Gaussian_filter')
    ax1.imshow(img_new.astype(np.uint8), cmap='gray')
    ax1.axis('off')
    ...
    ax2.title.set_text('sobel_kernel')
    ax2.imshow(img_tidu.astype(np.uint8), cmap='gray')
    ax2.axis('off')
    ...
    ax3.title.set_text('img_yizhi')
    ax3.imshow(img_yizhi.astype(np.uint8), cmap='gray')
    ax3.axis('off')
    ...
    ax4.title.set_text('canny_img')
    ax4.imshow(img_yizhi.astype(np.uint8), cmap='gray')
    ax4.axis('off')  # 关闭坐标刻度值

    img_cv = cv2.imread("lenna.png", 1)
    gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
    canny_cv = cv2.Canny(gray, 50, 200)
    ax5 = fig_1.add_subplot(2,3,5)
    ax5.title.set_text('canny_cv')
    ax5.imshow(canny_cv, cmap='gray')
    ax5.axis('off')  # 关闭坐标刻度值
    
    ax6 = fig_1.add_subplot(2,3,6)
    ax6.title.set_text('yuantu')
    ax6.imshow(img, cmap='gray')
    ax6.axis('off')  # 关闭坐标刻度值
